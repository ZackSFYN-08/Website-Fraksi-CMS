...
onMounted(async () => {
  try {
    const data = await api.getAspirationArticle(route.params.documentId)
    article.value = data || null
    // Fetch related articles (latest 3 excluding current)
    const all = await api.getAspirationArticles({ sort: 'date:desc', 'pagination[limit]': 4, populate: '*' })
  ...
  <div class="berita-detail-view">
    ...
    <div v-else-if="!article" class="empty-state container" data-reveal="fade-up">
      <div class="empty-card glass-card">
        ...
        <router-link to="/aspirasi/berita" class="btn btn-primary">← Kembali ke Berita Aspirasi</router-link>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <section class="container breadcrumb-container" data-reveal="fade-down">
        <div class="breadcrumb-glass glass-card">
          ...
          <router-link to="/aspirasi/berita">Berita Aspirasi</router-link>
          <i class="fas fa-chevron-right"></i>
      ...
      <main class="container article-layout">
        ...
        <aside class="article-sidebar">
          ...
          <router-link to="/aspirasi/berita" class="btn btn-dark btn-all-news" data-reveal="fade-left" data-reveal-delay="400">
            <i class="fas fa-th-list"></i> Lihat Semua Berita Aspirasi
          </router-link>
        </aside>
      </main>
    </template>
